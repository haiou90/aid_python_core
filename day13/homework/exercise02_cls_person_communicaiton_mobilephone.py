...
""" 小明使用手机打电话
    要求:增加座机,卫星电话时不影响小明.
    写出案例中体现的面向对象三大特征和六大原则.
        封装:创建Person MobilePhone LandLine三个类
        继承:创建Communication，抽象具体工具，统一行为，隔离Person与具体通信工具 MobilePhone LandLine
        多态:创建时调用父  运行时执行子
            调用父：人类调用通信工具 重写子：具体工具重写通信工具 创建子：创手机对象
            彰显个性 实现变化
        开闭原则:增加功能，卫星电话不影响人类
        单一职责:人类负责，手机负责，座机负责
        依赖倒置:人类调用通信工具，不调用具体通信工具
        组合复用:组合关系联系了人类与通信工具的变化
                继承关系统一了手机类 座机类
                
        里氏替换:人类打电话的形参是通信工具，调用时传递的各种具体通信工具
        迪米特:人类与具体通信工具低耦合
            各种通信工具之间低耦合
"""
class Person:
